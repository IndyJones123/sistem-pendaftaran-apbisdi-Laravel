        ...
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
          ...
          <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200">
            <div>
              ...
              <p class="text-sm text-gray-600">
                Tenggat Waktu Sertifikat Semua PT
              </p>
            </div>
            <!-- Search Form -->
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
              @foreach($sertifikatKaprodiData as $data)
              <tr class="bg-white hover:bg-gray-50">
                <td class="size-px whitespace-nowrap">
                  <a class="block relative z-10" href="#">
                    <div class="px-6 py-2">
                      <div class="block text-sm text-blue-600 decoration-2 hover:underline">{{$data->pt ? $data->pt->namapt : 'N/A'}}</div>
                    </div>
                  </a>
                </td>
                <td class="h-px w-20 min-w-20">
                  <a class="block relative z-10" href="#">
                    <div class="px-6 py-2">
                      <p class="text-sm text-gray-500">{{$data->pt ? $data->pt->nidn : 'N/A'}}</p>
                    </div>
                  </a>
                </td>
                <td class="size-px whitespace-nowrap">
...
<div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-t border-gray-200">
  <div>
    <p class="text-sm text-gray-600">
      <span class="font-semibold text-gray-800">{{ $sertifikatKaprodiData->total() }}</span> results
    </p>
  </div>
  ...
  <div>
    <div class="inline-flex gap-x-2">
      ...
      @if($sertifikatKaprodiData->onFirstPage())
      ...
      @else
        <a href="{{ $sertifikatKaprodiData->previousPageUrl() }}" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50">
          <svg class="flex-shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>
          Prev
        </a>
      @endif
      ...
      <!-- Next Page Button -->
      @if($sertifikatKaprodiData->hasMorePages())
        <a href="{{ $sertifikatKaprodiData->nextPageUrl() }}" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50">
          Next
